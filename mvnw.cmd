...
IF NOT %WRAPPER_SHA_256_SUM%=="" (
    powershell -Command "& {$hash = (Get-FileHash -Path '%WRAPPER_JAR%' -Algorithm SHA256).Hash.ToLower(); if ($hash -ne '%WRAPPER_SHA_256_SUM%') { Write-Error 'Error: Failed to validate Maven wrapper SHA-256, your Maven wrapper might be compromised.'; exit 1; }; }"
    if ERRORLEVEL 1 goto error
)
@REM End of SHA-256 validation
